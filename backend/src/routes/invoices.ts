...
import { User, Invoice, Patient, Appointment } from "@workspace/db";
...
router.patch("/invoices/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  // When an invoice is paid and it's linked to an appointment, update appointment status to "confirmed"
  if (updates.status === "paid" && invoice.appointmentId) {
