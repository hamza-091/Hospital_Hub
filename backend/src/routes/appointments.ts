...
import { User, Appointment, Doctor, Patient, Invoice } from "@workspace/db";
...
router.post("/appointments", requireAuth, async (req, res): Promise<void> => {
  ...
  // Create the appointment with "pending" status (meaning pending payment)
  ...
  const consultationFee = doctor?.consultationFee ?? 1500;

  // Auto-create an invoice for this appointment
  const lineItems = [{
    description: `Consultation Fee - Dr. ${(await User.findById(doctor?.userId).lean() as any)?.name ?? "Doctor"}`,
    quantity: 1,
    unitPrice: consultationFee,
    total: consultationFee,
  }];
  const subtotal = consultationFee;
  const tax = 0; // No tax for consultation
