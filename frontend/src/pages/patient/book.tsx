...
import { Stethoscope, Calendar, Clock, CheckCircle, Receipt, ArrowRight } from "lucide-react";
...
export default function PatientBook() {
  ...
  const handleBook = () => {
    ...
    createAppt.mutate(
      ...
      {
        onSuccess: () => {
          toast({ title: "Appointment booked!", description: "An invoice has been created. Please complete payment." });
            ...
            <div className="rounded-lg border p-4 space-y-2 bg-muted/20">
              ...
              {doctor?.consultationFee && (
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span><strong>Fee:</strong> {formatPKR(doctor.consultationFee)}</span>
                </div>
              )}
            </div>
      ...
      <Dialog open={showPaymentPopup} onOpenChange={setShowPaymentPopup}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Your appointment has been booked successfully! An invoice has been generated for this appointment.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4">
              <p className="text-sm font-medium text-yellow-800 mb-1">⚠️ Pending Payment</p>
              <p className="text-xs text-yellow-600">
                Your appointment status is <strong>Pending Payment</strong>. Please complete the payment from the Invoices section to confirm your appointment. The doctor will confirm your appointment after payment verification.
              </p>
            </div>
            <div className="flex gap-3 justify-end">
              <Button variant="outline" onClick={() => { setShowPaymentPopup(false); setLocation("/patient/appointments"); }}>
                View Appointments
              </Button>
              <Button onClick={() => { setShowPaymentPopup(false); setLocation("/patient/invoices"); }} className="gap-2">
